...
def _extend_metrics_to_tuple(outputs: list[list], metric_values: tuple[torch.Tensor], each_metric_value_process = _return_none):
    ...
    final_metrics = tuple((each_metric_value_process(metric_value), output.append(metric_value))[0] for output, metric_value in zip(outputs, metric_values))
    return final_metrics


def _extend_metrics_to_dict(outputs: dict[str, list], metric_values: dict[str, torch.Tensor], each_metric_value_process = _return_none):
    final_metrics = {key : (each_metric_value_process(metric_value), _dict_key_list(outputs, key).append(metric_value))[0] for key, metric_value in metric_values.items()}
    return final_metrics


def _extend_metrics_to_single(outputs: list[torch.Tensor], metric_values: torch.Tensor, each_metric_value_process = _return_none):
    outputs.append(metric_values)
    final_metrics = each_metric_value_process(metric_values)
    return final_metrics


def _clear_outputs_for_tuple(outputs: list[list], final_method: MethodType):
    gathered_outputs = tuple((torch.cat(output), output.clear())[0] for output in outputs if len(output) > 0)
    if len(gathered_outputs) > 0:
        final_method(*gathered_outputs)


def _clear_outputs_for_dict(outputs: dict[str, list], final_method: MethodType):
    gathered_outputs = {key : (torch.cat(output), output.clear())[0] for key, output in outputs.items() if len(output) > 0}
    if len(gathered_outputs) > 0:
        final_method(gathered_outputs)
...
def _clear_outputs_for_single(outputs: list, final_method: MethodType):
    if len(outputs) > 0:
        gathered_outputs = torch.cat(outputs)
        outputs.clear()
        final_method(gathered_outputs)
...
class ModelRunner(pl.LightningModule):
    ...
    def __training_step_for_tuple(self, metric_values):
        ...
        mean_metrics = _extend_metrics_to_tuple(self.__training_step_outputs, metric_values, torch.mean)
    ...
    def __training_step_for_dict(self, metric_values):
        self.process_training_step_metrics(metric_values)
        mean_metrics = _extend_metrics_to_dict(self.__training_step_outputs, metric_values, torch.mean)
        self.process_training_step_mean_metrics(mean_metrics)
    ...
    def __training_step_for_single(self, metric_values):
        self.process_training_step_metrics(metric_values)
        mean_metrics = _extend_metrics_to_single(self.__training_step_outputs, metric_values, torch.mean)
        self.process_training_step_mean_metrics(mean_metrics)
        loss = mean_metrics
        return loss
    ...
    def __validation_step_for_tuple(self, metric_values):
        ...
        _extend_metrics_to_tuple(self.__validation_step_outputs, metric_values)
    
    def __validation_step_for_dict(self, metric_values):
        self.process_validation_step_metrics(metric_values)
        _extend_metrics_to_dict(self.__validation_step_outputs, metric_values)
    
    def __validation_step_for_single(self, metric_values):
        self.process_validation_step_metrics(metric_values)
        _extend_metrics_to_single(self.__validation_step_outputs, metric_values)
    ...
    def __test_step_for_tuple(self, metric_values):
        ...
        _extend_metrics_to_tuple(self.__test_step_outputs, metric_values)
    
    def __test_step_for_dict(self, metric_values):
        self.process_test_step_metrics(metric_values)
        _extend_metrics_to_dict(self.__test_step_outputs, metric_values)
    
    def __test_step_for_single(self, metric_values):
        self.process_test_step_metrics(metric_values)
        _extend_metrics_to_single(self.__test_step_outputs, metric_values)
